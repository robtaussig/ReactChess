...
/***/ function(module, exports) {
	...
	var central = function central(pos) {
	  var centralVert = pos % 10;
	  var centralHor = Math.floor(pos / 10);
	  var vertDist = 1 / (Math.abs(4.5 - centralVert) * 2);
	  var horDist = 1 / (Math.abs(4.5 - centralHor) * 2);
	  return (vertDist + horDist) / 2;
	};
	
	var numMoves = function numMoves(pos, board) {};
	
	var pawnPosition = function pawnPosition(pos, board) {
	  return central(pos) * MATERIAL[board[pos]];
	};
	
	var rookPosition = function rookPosition(pos, board) {
	  return numMoves(pos, board);
	};
	
	var inBounds = function inBounds(pos) {
	  return pos > 9 && pos < 90 && pos % 10 !== 0 && pos % 10 !== 9;
	};
	
	var knightPosition = function knightPosition(pos, board) {
	  return central(pos) * MATERIAL[board[pos]];
	};
	
	var bishopPosition = function bishopPosition(pos, board) {};
	
	var queenPosition = function queenPosition(pos, board) {};
	
	var kingPosition = function kingPosition(pos, board) {};
	
	var piecePosition = function piecePosition(pos, board) {
	  switch (board[pos].toLowerCase()) {
	    case 'p':
	      return pawnPosition(pos);
	    case 'n':
	      return knightPosition(pos);
	    case 'r':
	      return rookPosition(pos, board);
	    default:
	      return 0;
	  }
	};
	...
	var BoardEval = function () {
	  ...
	  _createClass(BoardEval, [{
	    ...
	    value: function evaluateMove(board, move, depth) {
	      var newBoard = this.ai.makeMove(move, board);
	      var score = this.evaluateBoard(newBoard, 'b');
	    }
	  }, {
	    key: 'evaluateBoard',
	    value: function evaluateBoard(board, color) {
	      ...
	      for (var i = 0, n = board.length; i < n; i++) {
	        ...
	        positionalScore += piecePosition(this.ai, i, board);
	      }
...
/***/ }
/******/ ]);
//# sourceMappingURL=33863c565495de77c8c0.worker.js.map