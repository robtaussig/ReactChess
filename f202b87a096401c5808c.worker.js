	...
	var Ai = function () {
	  ...
	  _createClass(Ai, [{
	    ...
	    value: function kingMoves(position) {
	      ...
	      allMoves = allMoves.concat(this.checkCastle(position));
	      return allMoves;
	    }
	  }, {
	    key: 'checkCastle',
	    value: function checkCastle(position) {
	      var moves = [];
	      var color = this.board[position] === 15 ? 'b' : 'w';
	      if (this.specialMoves.queenSideCastle) {
	        if (this.emptyBetween(position, 12) && color === 'b') {
	          moves.push([position, 13]);
	        } else if (this.emptyBetween(position, 82)) {
...
/***/ }
/******/ ]);
//# sourceMappingURL=f202b87a096401c5808c.worker.js.map