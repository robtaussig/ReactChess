...
export function move(toX, toY, from = board.selectedSquare) {
  ...
  if (start[2] === 'p' && Math.abs(selected[1] - toY) === 2) {
    MoveRules.checkEnPassant(toX, toY, selected, board.pieces);
  } else if (start[2] === 'k') {
    if (Math.abs(toX - selected[0]) === 2) {
      MoveRules.castleRook(toX > selected[0] ? 5 : 3, selected[1],board.pieces);
    }
    ...
    MoveRules.captureEnPassant(toX, selected[1],board.pieces);
  }
